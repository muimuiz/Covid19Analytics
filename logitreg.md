...
$\operatorname{sigmoid}$ の積分が $\operatorname{softplus}$ であり、その逆関数 $\operatorname{logit}$ の積分は（符号を変えた）シャノン・エントロピーとなる。
...
D & = \left(\ (t_{S1}, t_{E1}, n_1, k_1),\ (t_{S2}, t_{E2}, n_2, k_2), \cdots, (t_{SN}, t_{EN}, n_N, k_N)\ \right)\\
  & \operatorname{where}\quad t_{Si} < t_{Ei}, \quad k_i, n_i \in \Z_{\geq 0},\ 0 \leq k_i \leq n_i, \quad i = 1, 2, \cdots, N.
\end{split}\end{equation*}
$$

ただし、$N \in \Z_{> 0}$ はビンの総数を表す。
...
__［定義］__
$p*$ に対して、次の関係で結ばれるロジットを $\lambda^\star$ と表す。
...
\end{alignat*}\tag{B2}\end{equation}
$$
